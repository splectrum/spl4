...
- **Entering:** /src rebased from caller's frame
  (/projects/06/) to mc.core's frame (root) →
  /projects/06/src
...
root. /src inside a protocol at /projects/06/ means
/projects/06/src — but the protocol writes /src.

**Design invariant:** every protocol reasons from a root
node, under all circumstances. A protocol at repo root
and one at /projects/06/ write the same code — absolute

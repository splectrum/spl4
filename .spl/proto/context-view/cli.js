#!/usr/bin/env node
import { scan, formatScan } from './scan.js';
import { buildPrompt, writePrompt, invoke } from './haiccer.js';
import { persist } from './persist.js';

const [, , command, ...args] = process.argv;

function help() {
  console.log(`context-view — context description generator

Commands:
  scan        Print scan result to stdout
  prompt      Generate haiccer prompt (saves to .context-view/)
  sync        Full pipeline: scan → haiccer → persist CONTEXT.md
  help        Show this help`);
}

try {
  switch (command) {
    case 'scan': {
      const result = await scan();
      console.log(formatScan(result));
      break;
    }
    case 'prompt': {
      const result = await scan();
      const prompt = buildPrompt(result);
      const path = writePrompt(prompt);
      console.log(`Prompt written: ${path}`);
      break;
    }
    case 'sync': {
      const result = await scan();
      const prompt = buildPrompt(result);
      const promptPath = writePrompt(prompt);
      console.log('Invoking haiccer...');
      const content = invoke(promptPath);
      persist(content);
      console.log(`Updated: ${process.env.SPL_ROOT}/CONTEXT.md`);
      break;
    }
    case 'help':
    case undefined:
      help();
      break;
    default:
      console.error(`Unknown command: ${command}`);
      help();
      process.exit(1);
  }
} catch (err) {
  console.error(err.message);
  process.exit(1);
}
